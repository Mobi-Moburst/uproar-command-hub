 ...
 * Never import this from browser code: it reads LOVABLE_API_KEY and handles
 * per-user lovack_* connection keys.
 */
...
export async function callAsAppUser({
  ...
  return await fetch(`${gatewayBaseUrl}/${connectorId}${normalizedPath}`, { ...init, headers });
}

/** A 401 whose body type starts with "credential_" means this user must reconnect. */
