...
export function ReportHero({ clientName, teamName, periodLabel }: ReportHeroProps) {
  ...
  const { logoUrl } = useClientLogo(clientName);

  return (
    <header className="relative overflow-hidden gradient-brand px-6 py-20 text-white print:py-12">
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="flex items-start gap-6">
          {/* Client logo */}
          {logoUrl && (
            <div className="hidden sm:flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-white/15 backdrop-blur-sm p-2">
              <img src={logoUrl} alt={`${clientName} logo`} className="h-full w-full object-contain" />
            </div>
          )}
